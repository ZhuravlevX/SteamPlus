  ...
  try {
    ...
    if (reviewAwardCtn) {
      ...
      awardDivs.forEach(div => {
        ...
        let points = 0;
        if (pointMatch) {
          points = parseInt((pointMatch[1] || pointMatch[2]).replace(/\s|,/g, ''), 10);
        }
        let users = 1;
      ...
      if (totalPoints > 0) {
        ...
        pointsRow.innerHTML = `<td><span style="color: #6088BB;">${formatPoints}</span></td>
                    <td>${earnedPoints}<span class="sub">${onlyYou}</span></td>`;
