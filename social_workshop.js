(function replaceSocialIcons() {
    // SVG path data for YouTube, Twitter (X), Facebook, Reddit, Sketchfab
    ...
    // Helper to create the SVG element
    function createSVGIcon(svgPath, viewBox = "0 0 1024 1024", width = 20, height = 20) {
    ...
    // YouTube
    const youtubeSelector = 'img.toolsIcon[src="https://community.cloudflare.steamstatic.com/public/images//social/youtube.png"]';
    document.querySelectorAll(youtubeSelector).forEach(img => {
    ...
    // Twitter (X)
    const twitterSelector = 'img.toolsIcon[src="https://community.cloudflare.steamstatic.com/public/images//social/twitter.gif"]';
    document.querySelectorAll(twitterSelector).forEach(img => {
    ...
    // Facebook
    const facebookSelector = 'img.toolsIcon[src="https://community.cloudflare.steamstatic.com/public/images//social/facebook.gif"]';
    document.querySelectorAll(facebookSelector).forEach(img => {
    ...
    // Reddit
    const redditSelector = 'img.toolsIcon[src="https://community.cloudflare.steamstatic.com/public/images//social/reddit.gif"]';
    document.querySelectorAll(redditSelector).forEach(img => {
    ...
    // Sketchfab
    const sketchfabSelector = 'img.toolsIcon[src="https://community.cloudflare.steamstatic.com/public/images//social/sketchfab.png"]';
    document.querySelectorAll(sketchfabSelector).forEach(img => {
    ...
    (function removePolycount() {
        ...
        document.querySelectorAll('div.box').forEach(box => {
            ...
            if (titleEl && titleEl.textContent.trim() === "Страница на Polycount") {
                box.remove();
            }
        });
    })();
    ...
    (function replaceTwitterWordEverywhere() {
        ...
        if (form) {
            form.querySelectorAll(".box").forEach(box => {
                ...
                if (title && title.textContent.includes("Твиттере")) {
                    title.textContent = title.textContent.replace(/Твиттере/g, "X");
                }
                if (desc && desc.textContent.includes("Твиттере")) {
                    desc.textContent = desc.textContent.replace(/Твиттере/g, "X");
                }
            });
        }
        // В кнопках .general_btn.panel_btn
        document.querySelectorAll('a.general_btn.panel_btn').forEach(btn => {
            btn.childNodes.forEach(node => {
                if (node.nodeType === Node.TEXT_NODE && node.textContent.includes("Твиттере")) {
                    node.textContent = node.textContent.replace(/Твиттере/g, "X");
                }
            });
        });
    })();
